...
bool thread_comp_prio(const struct list_elem *elem1, const struct list_elem *elem2, void *aux UNUSED);
...
/*Additional function -Thread comparison for priority*/
bool thread_comp_prio (const struct list_elem *elem1, const struct list_elem *elem2, void *aux UNUSED)
{
  struct thread *t1 , *t2;
...
{
  list_sort(&ready_list, thread_comp_prio, 0);
}
...
thread_set_priority (int new_priority) 
{
  ...
  //thread_current ()->priority = new_priority;
 /* printf("Current - Prio: %d", thread_current ()->priority);
  printf("Max - Prio: %d", list_entry(list_max(&ready_list, thread_comp_prio, 0), struct thread, elem)->priority);
  if( new_priority < list_entry(list_max(&ready_list, thread_comp_prio, 0), struct thread, elem)->priority)
  {*/

     /* yield current thread if priority changed*/
    // thread_yield();
  //}
...
/* sort ready list before popping from ready list*/
 list_sort(&ready_list, thread_comp_prio ,NULL);
 // static struct thread *rl_max;
 // static struct list_elem *lm;
